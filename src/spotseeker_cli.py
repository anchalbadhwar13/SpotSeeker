# #!/usr/bin/env python3
# """
# SpotSeeker CLI — Predict parking spot availability by zone.
# Usage: python spotseeker_cli.py
# """
...
# MODEL_PATH = os.path.join("spotseeker_final_nn.joblib")
# TRAIN_PATH = os.path.join("processed_data", "X_train.csv")
...
# def prompt_menu(title, options: dict, label="Choice") -> tuple:
#     """Print a numbered menu and return (key, value)."""
...
# def status_label(prob_available: float) -> tuple:
#     """Return (status string, colour) from probability of being available."""
...
# def build_feature_row(hour, day_of_week, weather, month=4, temp_c=12.0,
#                       duration_min=60, special_event=0) -> pd.DataFrame:
#     row = {
#         "Hour":             hour,
#         "Day_of_Week":      day_of_week,
#         "Month":            month,
#         "Precipitation":    WEATHER_PRECIPITATION[weather],
#         "Special_Event":    special_event,
#         "Temperature_C":    temp_c,
#         "Parking_Duration_Min": duration_min,
#         "Weather_Condition_Heavy Rain": 1 if weather == "Heavy Rain"  else 0,
#         "Weather_Condition_Light Rain": 1 if weather == "Light Rain"  else 0,
#         "Weather_Condition_Overcast":   1 if weather == "Overcast"    else 0,
#         "Weather_Condition_Sunny":      1 if weather == "Sunny"       else 0,
#         # Cloudy = all zeros (drop_first baseline)
#     }
...
#     # Load model + refit scaler on training data
#     try:
...
#         print(f"{RED}Error loading model files: {e}{RESET}")
#         print("Make sure you run this script from the SpotSeeker root directory.")
#         sys.exit(1)

#     while True:
#         # ── Inputs ───────────────────────────────────────────────────────────
#         print(f"{BOLD}─── Step 1: Day of week ───────────────────────{RESET}")
#         _, day_val  = prompt_menu("", DAYS, label="Day")
#         day_num, day_name = day_val

#         hour = prompt_hour()

#         print(f"\n{BOLD}─── Step 2: Weather ───────────────────────────{RESET}")
#         _, weather  = prompt_menu("", WEATHER_OPTIONS, label="Weather")

#         print(f"\n{BOLD}─── Step 3: Zone ──────────────────────────────{RESET}")
#         zone_choice, _ = prompt_menu("", ZONES, label="Zone")
#         zone_key    = ZONES[zone_choice]
#         capacity    = ZONE_CAPACITIES[zone_key]

#         # ── Predict ───────────────────────────────────────────────────────────
#         X_raw    = build_feature_row(hour, day_num, weather)
#         X_scaled = scaler.transform(X_raw)
#         proba    = model.predict_proba(X_scaled)[0]         # [P(Available), P(Unavailable)]
#         p_avail  = proba[list(model.classes_).index("Available")]
